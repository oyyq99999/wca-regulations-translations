...
- 3h++) [举例] 超常的修改包括：新的转动方式，无法进行常规的转动，可以看到更多的块，可以看到背面的颜色，自动转动，更多或不同的还原状态。
    ...
    - 3h4+) [澄清] 魔表可以通过对外观的修改增强其具体朝向的辨识度，因为传统的魔表设计已经提供了这些信息。
        - 3h4a) 视觉效果的修改（例如自定义背景图），由WCA代表决定。任何修改必须能清楚地看到各个时钟的12个小时，并且清楚的指明和原有插图一致的12点方向。
...
- 9f) 一轮比赛的比赛结果按以下方式衡量和记录：
    ...
    - 9f9) 在“5次取去尾平均”的比赛形式中，DNF或DNS视为最坏成绩。如果一个选手有不止一个DNF或DNS，则他该轮的平均成绩为DNF。
...
- 10i) 魔表的还原状态是指所有18个内表盘上的指针都已指向12点。
    - 10i1) 内表盘上没有清楚指向某个小时标志的指针被当作指向最近的小时标志。
